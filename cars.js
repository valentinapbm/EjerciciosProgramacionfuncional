const cars = [ { brand: 'Mazda', price: 5241, model: 1989 },
...
function ferraris (array){
    const filterArray=array.filter(function(car){
...
function nineties (array){
    const filterArray=array.filter(function(car){
        return car.model>=1980 && car.model<=1990;
    })
    return filterArray;
}
...
function list (array){
    const nuevoArray=array.map(function(car){
...
function bmwSum (array){
    ...
    const nuevoArray=filterArray.map(function(car){
        return car.price;
        
    })
    const sumTotal= nuevoArray.reduce(function(a,b){
        return (a + b)
    }) 
    return sumTotal;
}
